...
COMPOSITE_SHADOW_VERSION = 16  # Session 3.10 — robustness_bench paraphrase family (memorization defense). Pre-v16 records used a rotation pool of 7 perturbations that all left the inner problem text byte-identical to the canonical math item — they only prepended/appended wrapper instructions like "Solve the following problem.\n\n{p}" or "{p}\n\nProvide a final answer." A miner who indexed GSM8K / MATH-500 problems by exact-string or substring lookup would pass robustness_bench unchanged because the lookup key (the canonical problem text) survives every wrapper. v16+ adds two paraphrase-family perturbations that mutate the problem text itself — ``instruction_synonym`` (word-boundary synonym swap on a math-domain-safe table: find↔determine↔compute, total↔sum, each↔every, etc.) and ``imperative_to_question`` (rewrite ``Find the area of X.`` as ``What is the area of X?``). The picker stratifies so at least one paraphrase fires every round, preventing "wrapper-only" rounds. Mixing v15 and v16 robustness scores would let a memorizer who only passed wrapper-rounds inherit the crown via an inflated robustness_bench axis. v15 prompt-injection defense and v14 code_bench auto-indent fix carry forward unchanged.
